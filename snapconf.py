...
INTRON_TYPE_HEADER_MAP={'snaptron_id':int,'chromosome':str,'start':int,'end':int,'length':int,'strand':str,'annotated?':int,'left_motif':str,'right_motif':str,'left_annotated?':int,'right_annotated?':int,'samples':str,'read_coverage_by_sample':str,'samples_count':int,'coverage_sum':int,'coverage_avg':float,'coverage_median':float,'source_dataset_id':str}
...
for (i,field) in enumerate(INTRON_HEADER_FIELDS):
   ...
   INTRON_TYPE_HEADER_.append(str(INTRON_TYPE_HEADER_MAP[field]))
