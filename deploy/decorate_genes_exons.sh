...
#6) path/prefix of exon counts file with raw tab-delimited exon expression formatted: gene_id,bp_length,chrm,start,end,tab delimited list of summarized raw read counts for samples
...
cat ${1} | pypy ${p}/../annotations/process_genes_exons.py --annotation ${2} --sample-source ${3} --sample-metadata ${4} --annot-type gene --with-coords --as-ints | sort -t'	' -k2,2 -k3,3n -k4,4n | ${p}/compute_stats_per_record.sh ${5} | tee ./import | bgzip > ${3}.genes.tsv.bgz &
...
#this will sort the output by coordinate, but because the snaptron_id has already been assigned by process_genes_exons.py, it will not be in order
cat ${1}.exons | pypy ${p}/../annotations/process_genes_exons.py --annotation ${2} --sample-source ${3} --sample-metadata ${4} --annot-type exon --with-coords --as-ints | sort -t'	' -k2,2 -k3,3n -k4,4n | ${p}/compute_stats_per_record.sh ${5} | tee ./import | bgzip > ${3}.exons.tsv.bgz &
sqlite3 $db -cmd '.separator "\t"' ".import ./import intron"
sqlite3 $db < ${p}/snaptron_schema_index.sql
