...
perl ../join_junction_and_sample_md.pl ${report} sample_counts.tsv | cut -f 1,2,4- | cut -f 1-11,13,16- > sample_counts.tsv.formatted_md.tsv

echo 'rail_sample_id	SRR	REL_Gene_Splice_Sense_Frac	Gene_Splice_Sense_Frac	REL_Splice_Sense_Frac	REL_Total_Frac	REL_Cov_Frac	REL_Spot_Frac	REL_Gene_Splice_Sense_Count	REL_count	Gene_Splice_Sense_Count	REL_Splice_Sense_Count	Total_count	Rel_Cov	Total_Cov	spots	bases	avg_read_length	paired	platform_model	library_strategy	library_source	library_selection	library_name	analyte_type	body_site	histological_type	tumor	sex	disease	scientific_name	sample_name' >gtex_sarven_REL_report_by_sample_counts.sorted.tsv
